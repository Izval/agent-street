...
# IVL rebalance planner — READ-ONLY: consulta el motor IVL (api.zvlint.com) y simula
# el mint v3 contra el pool en vivo (eth_call, sin firmar ni gastar). Es la skill
# distintiva del flagship. La EJECUCIÓN con firma vive en rebalance.execute_rebalance
# (código fijo, NUNCA un tool) — igual que signing.py, el dinero nunca pasa por el LLM.
from rebalance import ivl_rebalance_plan

LLM_READ_TOOLS = [
    # --- IVL rebalancing (flagship skill; read-only planner) ---
    FunctionTool(ivl_rebalance_plan),
