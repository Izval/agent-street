 ...
 *
 * One row of the 7 aisle triggers (`AISLES`) + a SINGLE shared mega-panel whose
 * content swaps per aisle (`openAisle` state) — you navigate *between* aisles
 * inside one component, never 7 separate menus. Each trigger is a real
 * `<Link to="/aisle/:id">` (click navigates) that also opens the panel on
 * hover/focus. The panel shows the aisle's `categoriesInAisle` (with ★ on the
...
export function MegaMenu({
  ...
  const [open, setOpen] = useState<Aisle | null>(null);
  ...
  // Roving arrow-key movement between the aisle triggers.
  const onNavKeyDown = (e: React.KeyboardEvent) => {
    const idx = triggers.current.findIndex((t) => t === document.activeElement);
    if (idx < 0) return;
    let next = idx;
    if (e.key === "ArrowRight") next = (idx + 1) % AISLES.length;
    else if (e.key === "ArrowLeft") next = (idx - 1 + AISLES.length) % AISLES.length;
    else return;
    e.preventDefault();
    triggers.current[next]?.focus();
  };

  const openAisleDef = open ? AISLES.find((a) => a.id === open) ?? null : null;
  const subs = open ? categoriesInAisle(open) : [];
  ...
  return (
    ...
    >
        ...
        {AISLES.map((a, i) => {
          ...
          return (
            <Link
              ...
              className={
                "relative flex shrink-0 items-center gap-1.5 rounded-[6px] px-3 py-2 text-sm font-medium transition-colors " +
      ...
      {open !== null && openAisleDef && typeof document !== "undefined"
        ? createPortal(
            <div
              role="menu"
              aria-label={`${openAisleDef.label} categories`}
              onMouseEnter={cancelClose}
              onMouseLeave={scheduleClose}
              style={{
                position: "fixed",
                ...
                left: 0,
                right: 0,
                ...coverStyle(openAisleDef.id, openAisleDef.accent),
              }}
            ...
            >
              {/* Legibility scrim — darker on the left where the links sit,
                  fading right so the category art breathes. */}
              <span
                aria-hidden
              ...
              <div className="relative mx-auto grid max-w-[1440px] grid-cols-1 gap-8 px-4 py-7 md:grid-cols-[1fr_300px] md:px-8">
                ...
                <div>
                  <div className="mb-4 flex items-center gap-2 text-[11px] font-semibold uppercase tracking-[0.16em] text-white/60">
                    <span
                      ...
                      style={{ background: openAisleDef.accent }}
                    />
                    {openAisleDef.label}
                  </div>
                  <div className="grid grid-cols-1 gap-1 sm:grid-cols-2">
                    {subs.map((c) => (
                      <Link
                        ...
                        role="menuitem"
                        aria-current={
                          activeCategory === c.id ? "page" : undefined
                        }
                        onClick={() => setOpen(null)}
                        className={
                ...
                {/* Immersive aisle identity + CTA (over the art). */}
