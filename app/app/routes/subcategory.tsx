...
export function meta({ loaderData }: Route.MetaArgs) {
  const label = loaderData?.label ?? "Subcategory";
...
/** Fine-grained overrides for subcategories with their own voice. */
const SUBCATEGORY_TAGLINE: Partial<Record<Subcategory, string>> = {
  rebalancing:
...
function taglineFor(id: Subcategory, template: TemplateKind): string {
  return SUBCATEGORY_TAGLINE[id] ?? TEMPLATE_TAGLINE[template];
}
...
export async function loader({ params, request }: Route.LoaderArgs) {
  const id = params.id as Subcategory;
  if (!(SUBCATEGORIES as readonly string[]).includes(id)) {
  ...
  const result = await agents.list({ subcategory: id, page: 1, limit: POOL, search });
  ...
  const categoryDef = category ? CATEGORIES.find((a) => a.id === category) : undefined;
  const template = subcategoryTemplate(id);
  ...
  return {
    ...
    categoryLabel: categoryDef?.label ?? null,
    parentTo: category ? `/category/${category}` : null,
    accent: categoryDef?.accent ?? null,
    page,
    sort,
    search: search ?? "",
    featured,
...
export default function SubcategoryPage({ loaderData }: Route.ComponentProps) {
  const {
    subcategory,
    label,
    templateLabel,
  ...
  // The eyebrow carries the parent category + honest count; the slideshow is the hero.
  const eyebrow = `${categoryLabel ?? templateLabel} · ${total.toLocaleString(
    "en-US",
  )} agents`;
  ...
  const heroSlides: HeroSlide[] = [
    {
      ...
      title: label,
      subtitle: tagline,
      cover: `subcategory-${subcategory}`,
      accent: accent ?? undefined,
      ctaTo: parentTo ?? `/subcategory/${subcategory}`,
    },
  ];

  return (
    <AppShell activeSubcategory={subcategory}>
      {/* Subcategory hero. */}
          ...
          <CollectionCarousel title="Featured" accent={accent ?? undefined}>
            {featured.map((a) => (
              <AgentFeatureCard
                ...
                agent={a}
                accent={accent ?? undefined}
                eyebrow={a.subcategoryLabel ?? label}
              />
            ))}
          </CollectionCarousel>
        </div>
      ...
      <section className="mt-10">
        <div className="flex flex-wrap items-end justify-between gap-3">
          ...
          <div className="flex items-center gap-3">
            <SortTabs subcategory={subcategory} sort={sort} search={search} />
        ...
        {(page > 1 || hasMore) && (
          <div className="mt-8 flex items-center justify-between">
            <PageLink
              to={pageUrl(subcategory, page - 1, sort, search)}
              disabled={page <= 1}
            >
              ← Previous
            </PageLink>
    ...
    <div className="inline-flex rounded-full border border-border bg-surface p-0.5">
      {opts.map((o) => {
        ...
        return (
          <Link
            key={o.key}
            to={pageUrl(subcategory, 1, o.key, search)}
            aria-current={active}
            className={
              active
...
function pageUrl(
  ...
  const qs = p.toString();
  return `/subcategory/${subcategory}${qs ? `?${qs}` : ""}`;
}
