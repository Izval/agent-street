...
export function meta({ loaderData }: Route.MetaArgs) {
  ...
  return [{ title: q ? `«${q}» — Agent-Street` : "Buscar — Agent-Street" }];
}
    ...
    <AppShell>
      <header className="py-2">
        <h1 className="text-3xl font-bold">
          {q ? <>Resultados para «{q}»</> : "Buscar agentes"}
        </h1>
        {q && (
          <p className="mt-1 text-sm text-text-3">
            {total.toLocaleString("en-US")}{" "}
            {total === 1 ? "agente" : "agentes"} · datos onchain de 8004scan
          </p>
        )}
      </header>
      ...
      <Form method="get" className="mt-4 flex gap-2">
        <input
          ...
          name="q"
          defaultValue={q}
          placeholder="Buscar agentes en BNB Chain…"
      ...
      {!q ? (
        <p className="mt-8 rounded-lg border border-border bg-surface p-6 text-sm text-text-3">
          Escribe un término para buscar agentes por nombre o descripción.
        </p>
      ) : agents.length ? (
        ...
        <p className="mt-8 rounded-lg border border-border bg-surface p-6 text-sm text-text-3">
          Sin resultados para «{q}». Prueba con otro término o explora por
          categoría desde la barra lateral.
        </p>
      )}
    </AppShell>
  );
